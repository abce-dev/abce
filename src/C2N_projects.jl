...
function create_C2N_capex_timeline(
    db,
    conv_type,
    rxtr_type,
    base_pd,
...
function project_capex_profile(
    base_pd,
    lag,
    db,
    fc_pd,
    ...
    elseif status == "ongoing"
        ...
        project_current_status = Dict(
            :npp_ns_xtr => Dict(
                "cost_rem" => project_WIP_update[:npp_ns_xtr_cost_rem],
                "time_rem" => project_WIP_update[:npp_ns_xtr_time_rem],
            ),
            :npp_safety_xtr => Dict(
                "cost_rem" => project_WIP_update[:npp_safety_xtr_cost_rem],
                "time_rem" => project_WIP_update[:npp_safety_xtr_time_rem],
            ),
            :cpp_dnd => Dict(
                "cost_rem" => project_WIP_update[:cpp_dnd_cost_rem],
                "time_rem" => project_WIP_update[:cpp_dnd_time_rem],
            ),
            :cpp_wr => Dict(
                "cost_rem" => project_WIP_update[:cpp_wr_cost_rem],
                "time_rem" => project_WIP_update[:cpp_wr_time_rem],
            ),
            :cpp_nrc => Dict(
                "cost_rem" => project_WIP_update[:cpp_nrc_cost_rem],
                "time_rem" => project_WIP_update[:cpp_nrc_time_rem],
            ),
        )
    end
    ...
    capex_tl, capex_activity_schedule =
        convert_to_annual_capex_schedule(capex_activity_schedule)
...
function get_C2N_available_activities(
    conv_type,
    project_current_status,
    base_pd,
    lag,
...
function get_available_activities(project_current_status, conv_type)
    if conv_type == "greenfield"
        available_activities =
            get_available_activities_greenfield(project_current_status)
    elseif conv_type == "electrical"
        available_activities =
            get_available_activities_electrical(project_current_status)
    elseif conv_type == "steam_noTES"
        available_activities =
            get_available_activities_steam_noTES(project_current_status)
    elseif conv_type == "steam_TES"
        available_activities =
            get_available_activities_steam_TES(project_current_status)
    else
        println(
            string(
                "I don't recognize that conversion project type. ",
...
function get_available_activities_greenfield(project_current_status)
    all_activities =
        ["npp_ns_xtr", "npp_safety_xtr", "cpp_dnd", "cpp_wr", "cpp_nrc"]
    ...
    incomplete_activities = [
        all_activities[i] for i = 1:size(all_activities)[1] if
        !(all_activities[i] in completed_activities)
    ]
...
function get_available_activities_steam_TES(project_current_status)
    ...
    if (
        ("npp_safety_xtr" in completed_activities) &&
        !("cpp_dnd" in completed_activities)
    )
        push!(available_activities, "cpp_dnd")
    end
...
function allocate_funds_to_activities(
    activity_schedule,
    C2N_specs,
    project_current_status,
    unit_type_data,
...
function convert_to_annual_capex_schedule(capex_activity_schedule)
    ...
    transform!(
        capex_activity_schedule,
        [:cpp_dnd, :cpp_nrc, :cpp_wr, :npp_ns_xtr, :npp_safety_xtr] =>
            ((a, b, c, d, e) -> a .+ b .+ c .+ d .+ e) => :unscaled_capex,
    )
    ...
    for i = start_pd:end_pd
        subset = filter(
            [:state_start, :state_end] =>
                ((s_start, s_end) -> !(s_end <= i) && !(s_start >= i + 1)),
            capex_activity_schedule,
        )
